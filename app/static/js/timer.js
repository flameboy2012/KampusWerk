...
(function($){
   ...
   var TBTimer = function(config){
      ...
      this.specs = {
         rounds: config.specs.rounds || 6,
         rest: config.specs.rest || 3,
         work: config.specs.work || 7
      };
      ...
      //THIS next section updates the display ie remove millis
      ...
      this._doRound = function(rn){
         ...
         if(this.rounds[rn]){
           ...
           /* if(current.round.type == 'rest'){
               this.notify('Rest Now!');
               if(rn != 0){
                  this.sounds.play('end-round');
               }
            }else{
               this.notify('Hangboard Werk!');
               this.sounds.play('start');
            }
            */
         }else{
            this.notify('Good Session Punter!');
