...
def Hangboard():
    ...
    if form.validate_on_submit():
        timestamp=datetime.utcnow()
        werk = HangboardWerkout(current_user.nickname, request.form['board'], request.form['holds_used'], request.form['reps'], request.form['sets'], request.form['setrest'], request.form['arm_used'], request.form['hangtime'], request.form['resttime'], request.form['weight_kg'], timestamp, current_user.id)
        db.session.add(werk)
        db.session.commit()
...
def Profile():
    ...
    if form.validate_on_submit():
        ...
        flash_errors(form)
    return render_template("profile.html", form=form, bodyweight=Bodyweight.query.all())
...
@views.route("/werktime", methods=["GET","POST"])
def werktime():
